...
async def aggregate_results_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if state.final_response:
            ...
            # Preserve existing structured_output if it contains assessment data
            # (set by package orchestration or skill execution)
            existing_so = state.structured_output
            if isinstance(existing_so, dict) and (
                existing_so.get("status") in ("goal_selection", "missing_basic_info")
                or existing_so.get("modules")
            ):
                logger.info(f"Preserving structured_output (has {'status' if existing_so.get('status') else 'modules'})")
                return state
            # Create structured output for consistency
